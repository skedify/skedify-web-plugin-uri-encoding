...
export function encodeURIParameters(params) {
  const params_string = btoa(JSON.stringify(params));
  const idx_of_first_eqs = params_string.indexOf('=');
  const no_of_eqs = idx_of_first_eqs < 0
    ? 0
    : params_string.length - idx_of_first_eqs;
  return params_string.substring(0, params_string.length - no_of_eqs)
...
export function decodeURIParameters(encoded) {
  ...
  return JSON.parse(
    `${atob(encoded.substr(0, end) + repeat('=', codePointAt(encoded, end) - codePointAt(encoded, 0)))}`,
  );
}
